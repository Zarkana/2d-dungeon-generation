...
public class Corridor : Area
{
  ...
  public int endXPos;         // The x coordinate for the start of the corridor.
  public int endYPos;         // The y coordinate for the start of the corridor.
  ...
  public void SetupCorridor(Area origin, Area destination, bool firstCorridor)
  {
    this.origin = origin;
    ...
    //find the closest direction to the destination
    directions[0] = origin.closestDirection(destination);

    Tile originTile = new Tile();
    Tile destinationTile = new Tile();

    //TODO: Assess efficiency, it seems we are doing multiple things twice
    switch (directions[0])
    {
      // If the choosen direction is North (up)...
      case Direction.Up:
        //TODO: Implement randomness ... the starting position in the x axis can be random but within the width of the room.
        //startXPos = Random.Range(room.xPos, room.xPos + room.maxWidth - 1);
        originTile = origin.GetTopCenter();
        startXPos = (int)originTile.vectorTile.x;
        startYPos = (int)originTile.vectorTile.y;
        if(origin.isLeft(destination))
        {
          //TODO: Could be waste here, storing destination tile, idk
          directions[1] = Direction.Left;
          destinationTile = destination.GetRightCenter();
          corridorLength = GetDistance(originTile, destinationTile);
        }
        else //if right
        {
          directions[1] = Direction.Right;
          destinationTile = destination.GetLeftCenter();
          corridorLength = GetDistance(originTile, destinationTile);
        }
        endXPos = (int)destinationTile.vectorTile.x;
        endYPos = (int)destinationTile.vectorTile.y;        
        break;
      case Direction.Right:
        //startXPos = room.xPos + room.maxWidth;
        //startYPos = Random.Range(room.yPos, room.yPos + room.maxHeight - 1);
        originTile = origin.GetRightCenter();
        startXPos = (int)originTile.vectorTile.x;
        startYPos = (int)originTile.vectorTile.y;
        if (origin.isUp(destination))
        {
          directions[1] = Direction.Up;
          destinationTile = destination.GetBottomCenter();
          corridorLength = GetDistance(originTile, destinationTile);
        }
        else //if down
        {
          directions[1] = Direction.Down;
          destinationTile = destination.GetTopCenter();
          corridorLength = GetDistance(originTile, destinationTile);
        }
        endXPos = (int)destinationTile.vectorTile.x;
        endYPos = (int)destinationTile.vectorTile.y;
      ...
      case Direction.Down:
        ...
        //startYPos = room.yPos;
        originTile = origin.GetBottomCenter();
        startXPos = (int)originTile.vectorTile.x;
        startYPos = (int)originTile.vectorTile.y;
        if (origin.isLeft(destination))
        {
      ...
      case Direction.Left:
        ...
        originTile = origin.GetLeftCenter();
        startXPos = (int)originTile.vectorTile.x;
        startYPos = (int)originTile.vectorTile.y;
        if (origin.isUp(destination))
        {
    ...
    //TODO: generate corridor path with width and randomness variables
